...
model_name = "conv"
m = torch.nn.Conv2d(3, 3, 3, stride=2)
...
for i in range(3):
    for j in range(3):
        for k in range(3):
            f.write(str(weight[i][j][k][0]) + ' ' + str(weight[i][j][k][1]) + ' ' + str(weight[i][j][k][2]) + '\n')

input_shape = [1, 3, 224, 224]
input_data = torch.randn(input_shape)
scripted_model = torch.jit.trace(m, input_data).eval()
mod, params = relay.frontend.from_pytorch(scripted_model, shape_list)
...
for i, batch in enumerate(val_data):
    ...
    for j in range(batch_size):
        ...
        real_output = m(torch.tensor(data[j].reshape((1, 3, 224, 224)).astype(dtype)))
        after_inference = time.time()
        print("after_inference - before_inference")
        print(after_inference - before_inference)
        ...
        if target[j] in top5:
            top5_acc += 1
        # print("quant_output", quant_output)
        
        # print("real_output", real_output)
        # print("conv", m.weight.data.numpy())
        exit()
